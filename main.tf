...
resource "aws_db_instance" "wp_rds" {
  ...
  port                   = 3306
  db_name                = "wordpress"
  identifier             = "Wordpress-Workshop"
  username               = "wpadmin"
  password               = "wpadmin"
}

resource "aws_security_group" "wp_cache_client_sg" {
  name   = "WP Cache Client SG"
  vpc_id = aws_vpc.ha-web-app.id
}

resource "aws_security_group" "wp_cache_sg" {
  name   = "WP Cache Client SG"
  vpc_id = aws_vpc.ha-web-app.id
}

resource "aws_vpc_security_group_ingress_rule" "wp_cache_sg" {
  security_group_id = aws_security_group.wp_cache_sg.id
...
resource "aws_elasticache_subnet_group" "wp-ec" {
  name       = "Wordpress-Elasticache"
  subnet_ids = [aws_subnet.data-a.id, aws_subnet.data-b.id]
}

resource "aws_elasticache_cluster" "wp-memcached" {
  ...
  port                 = 11211
  network_type         = "ipv4"
  subnet_group_name    = aws_elasticache_subnet_group.wp-ec.name
  security_group_ids   = [aws_security_group.wp_cache_sg.id]
}
...
#   vpc_security_group_ids = [aws_security_group.wp_wordpress_sg.id, aws_security_group.db_client_sg.id, aws_security_group.wp_fs_client_sg.id, aws_security_group.wp_cache_client_sg.id]
